...
const encodeDecode = async () => {
  ...
  const readableStream: any = cipher.input ? fs.createReadStream(cipher.input, "utf8") : process.stdin;
  const writeableStream: any = cipher.output ? fs.createWriteStream(cipher.output, { flags: 'a+' } ) : process.stdout;
